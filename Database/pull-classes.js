...
module.exports.pullclasses = function(){
    return new provider.RSVP.Promise(function(resolve, reject) {
        ...
        //----------------------------------------------------------------------------------------------------+-----------------------------------------------------------+------
        provider.connection.query('SELECT classID, className, DATE_FORMAT(classDate, "%Y-%m-%d") AS classDate, startTime, EndTime  FROM classes WHERE classDate >= DATE_ADD(CURDATE(), INTERVAL 0 MONTH) AND classDate <= DATE_ADD(CURDATE(), INTERVAL 1 MONTH)', function(err, rows) {
